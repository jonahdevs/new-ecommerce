...
new #[Layout('layouts.checkout')] class extends Component
{
    ...
    public function mount(string $order): void
    {
        ...
        $orderModel = Order::where('reference', $order)
            ...
            ->with(['payment', 'items.product:id,image_path,name', 'user'])
            ->firstOrFail();
    ...
    {
        return Order::with(['payment', 'items.product:id,image_path,name'])->findOrFail($this->orderId);
    }
    ...
    public function initiateMpesa(): void
    {
        ...
        try {
            ...
            logger()->error('M-Pesa initiation failed on quote pay page', ['error' => $e->getMessage()]);
            $this->isProcessing = false;
        }
    }
...
<div>
    <x-slot:breadcrumbs>
        <flux:breadcrumbs class="container mx-auto px-4">
            ...
            <flux:breadcrumbs.item :href="route('customer.quotations.index')" wire:navigate>Quotations</flux:breadcrumbs.item>
    ...
    <x-slot:sidebar>
        <livewire:quote-order-summary :order-id="$this->orderId">
            <div class="px-4 py-2 text-center">
                <p class="text-[11px] font-bold uppercase tracking-widest text-on-surface-variant">Choose a payment method</p>
                <div class="mt-3 flex items-center justify-center gap-1.5 text-[10px] text-on-surface-variant font-bold uppercase tracking-widest">
                    <flux:icon.shield-check class="size-3" />
                    <span>SSL Encrypted & Secure</span>
                </div>
            </div>
        </livewire:quote-order-summary>
    </x-slot:sidebar>
    ...
    <flux:modal name="stk-waiting" class="max-w-sm p-0! rounded-none!">
        <div x-data="stkWaiting" x-init="init()">
            ...
            <div x-show="!timedOut" class="text-center p-7">
                ...
                <p class="text-[11px] text-on-surface-variant font-medium uppercase tracking-widest">
        ...
        Alpine.data('stripePayment', () => ({
            ...
            initStripe() {
                ...
                [this.cardNumber, this.cardExpiry, this.cardCvc].forEach(el => {
                    el.on('ready', () => { if (++readyCount === 3) this.ready = true; });
                    el.on('change', (e) => { this.errorMessage = e.error?.message ?? ''; });
                });
            },
                    ...
                    const { paymentIntent, error } = await this.stripe.confirmCardPayment(
                        this._clientSecret, {
                            payment_method: {
                                ...
                                billing_details: { name: this.cardholderName || undefined },
                            },
