...
<head>
    ...
    <title>{{ $subject }}</title>
    <style>
        /* Base styles integrated from Maizzle design */
        ...
        .border-brand { border: 1px solid #c02434; }
    </style>
</head>
            ...
            <td style="padding: 24px 36px;">
                ...
                <p style="margin: 0 0 32px; font-size: 16px; line-height: 24px; color: #475569;">
                    @if($newStatus === \App\Enums\OrderStatus::SHIPPED)
                        Exciting news! Your order <span style="font-weight: 600; color: #1e293b;">#{{ $order->reference }}</span> has been shipped and is currently in transit.
                    @elseif($newStatus === \App\Enums\OrderStatus::DELIVERED)
                        Great news! Your order <span style="font-weight: 600; color: #1e293b;">#{{ $order->reference }}</span> has been delivered. We hope you enjoy your purchase!
                    @elseif($newStatus === \App\Enums\OrderStatus::CANCELLED)
                        Your order <span style="font-weight: 600; color: #1e293b;">#{{ $order->reference }}</span> has been cancelled. If you have any questions, please contact our support team.
                    @else
                        Your order <span style="font-weight: 600; color: #1e293b;">#{{ $order->reference }}</span> status has been updated to <span style="font-weight: 700; color: #c02434; text-transform: uppercase;">{{ $newStatus->label() }}</span>.
                    @endif
                </p>

                <!-- Timeline -->
                @if($newStatus !== \App\Enums\OrderStatus::CANCELLED)
                    ...
                    <tr>
                        @php
                            $statuses = [
                                'Placed' => true,
                                'Confirmed' => true,
                                'Shipped' => in_array($newStatus, [\App\Enums\OrderStatus::SHIPPED, \App\Enums\OrderStatus::DELIVERED]),
                                'Delivered' => $newStatus === \App\Enums\OrderStatus::DELIVERED
                            ];
                        @endphp

                        @foreach($statuses as $label => $active)
                            <td style="width: 40px; text-align: center;">
                                <div style="height: 40px; width: 40px; margin: 0 auto; background-color: {{ $active ? '#c02434' : '#ffffff' }}; border: 2px solid {{ $active ? '#c02434' : '#e2e8f0' }}; border-radius: 9999px; line-height: 36px; text-align: center;">
                                    @if($active)
                                        <span style="color: #ffffff; font-weight: bold; font-size: 18px;">✓</span>
                                    @else
                                        <span style="color: #cbd5e1; font-weight: bold; font-size: 14px;">{{ $loop->iteration }}</span>
                                    @endif
                                </div>
                            </td>
                            @if(!$loop->last)
                                <td style="vertical-align: middle;">
                                    <div style="height: 4px; background-color: {{ $statuses[array_keys($statuses)[$loop->index + 1]] ? '#c02434' : '#e2e8f0' }};"></div>
                                </td>
                            @endif
                        @endforeach
                    </tr>
                    <tr>
                        @foreach($statuses as $label => $active)
                            <td style="text-align: center; padding-top: 12px;">
                                <p style="margin: 0; font-size: 11px; font-weight: bold; color: {{ $active ? '#c02434' : '#94a3b8' }}; text-transform: uppercase; letter-spacing: 1px;">{{ $label }}</p>
                            </td>
                            @if(!$loop->last)
                                <td></td>
                            @endif
                ...
                <table style="margin-bottom: 32px;" cellpadding="0" cellspacing="0" role="presentation">
                    <tr>
                        <td style="width: 50%; vertical-align: bottom;">
                            ...
                            <p style="margin: 0; font-size: 14px; font-weight: 600; color: #1e293b;">{{ $order->payment?->gateway_label ?? 'Online Payment' }}</p>
                        </td>
                        <td style="width: 50%;">
