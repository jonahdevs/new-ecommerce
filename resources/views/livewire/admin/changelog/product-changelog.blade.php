...
new #[Title('Product Changelog')] class extends Component {
    use WithPagination;
    ...
    {
        return Activity::query()->where('subject_type', Product::class)->where('subject_id', $this->product->id)->with('causer')->latest()->paginate(20);
    }

    public function render(): View
    {
        return view('livewire.admin.changelog.product-changelog');
    }
    ...
    public function formatValue(mixed $value, string $field): string
    {
        ...
        return match ($field) {
            ...
            'category_id' => $this->getCategoryName($value),
            'brand_id' => $this->getBrandName($value),
...
<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">Product Changelog</flux:heading>
            <flux:subheading>Change history for {{ $product->name }}</flux:subheading>
        </div>
        <flux:button :href="route('admin.catalog.products.edit', $product)" wire:navigate icon="arrow-left"
            variant="ghost">
            Back to Product
        </flux:button>
    </div>
