...
new #[Title('Order Changelog')] class extends Component {
    use WithPagination;
    ...
    {
        return Activity::query()->where('subject_type', Order::class)->where('subject_id', $this->order->id)->with('causer')->latest()->paginate(20);
    }

    public function render(): View
    {
        return view('livewire.admin.changelog.order-changelog');
    }
    ...
    public function formatValue(mixed $value, string $field): string
    {
        ...
        return match ($field) {
            'status' => \App\Enums\OrderStatus::from($value)->label(),
            'payment_status' => \App\Enums\PaymentStatus::from($value)->label(),
...
<div>
    <flux:breadcrumbs class="mb-2">
        ...
        <flux:breadcrumbs.item :href="route('admin.sales.orders.index')" wire:navigate>Orders
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">Order Changelog</flux:heading>
            <flux:subheading>Change history for Order #{{ $order->reference }}</flux:subheading>
        </div>
        <flux:button :href="route('admin.sales.orders.show', $order)" wire:navigate icon="arrow-left" variant="ghost">
            Back to Order
        </flux:button>
    </div>
