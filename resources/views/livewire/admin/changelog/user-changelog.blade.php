...
new #[Title('User Changelog')] class extends Component {
    use WithPagination;
    ...
    {
        return Activity::query()->where('subject_type', User::class)->where('subject_id', $this->user->id)->with('causer')->latest()->paginate(20);
    }

    public function render(): View
    {
        return view('livewire.admin.changelog.user-changelog');
    }
    ...
    public function formatValue(mixed $value, string $field): string
    {
        ...
        return match ($field) {
            'status' => \App\Enums\UserStatus::from($value)->label(),
...
<div>
    <flux:breadcrumbs class="mb-2">
        ...
        <flux:breadcrumbs.item :href="route('admin.access-control.roles.index')" wire:navigate>Roles
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">User Changelog</flux:heading>
            <flux:subheading>Change history for {{ $user->name }}</flux:subheading>
        </div>
        <flux:button :href="route('admin.access-control.users.edit', $user)" wire:navigate icon="arrow-left"
            variant="ghost">
            Back to User
        </flux:button>
    </div>
                ...
                <div class="flex flex-col items-center gap-3">
                    ...
                    <div>
                        ...
                        <flux:subheading class="mt-0.5">
                            Changes to this user will appear here.
                        </flux:subheading>
                    </div>
                </div>
            </div>
