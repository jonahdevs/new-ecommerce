...
new #[Title('Quote Changelog')] class extends Component {
    use WithPagination;
    ...
    {
        return Activity::query()->where('subject_type', Quote::class)->where('subject_id', $this->quote->id)->with('causer')->latest()->paginate(20);
    }

    public function render(): View
    {
        return view('livewire.admin.changelog.quote-changelog');
    }
    ...
    public function formatValue(mixed $value, string $field): string
    {
        ...
        return match ($field) {
            'status' => \App\Enums\QuoteStatus::from($value)->label(),
            'admin_notes' => $value ?: '—',
...
<div>
    <flux:breadcrumbs class="mb-2">
        ...
        <flux:breadcrumbs.item :href="route('admin.quotations.index')" wire:navigate>Quotes
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <flux:heading size="xl">Quote Changelog</flux:heading>
            <flux:subheading>Change history for Quote #{{ $quote->reference }}</flux:subheading>
        </div>
        <flux:button :href="route('admin.quotations.show', $quote)" wire:navigate icon="arrow-left" variant="ghost">
            Back to Quote
        </flux:button>
    </div>
                ...
                <div class="flex flex-col items-center gap-3">
                    ...
                    <div>
                        ...
                        <flux:subheading class="mt-0.5">
                            Changes to this quote will appear here.
                        </flux:subheading>
                    </div>
                </div>
            </div>
