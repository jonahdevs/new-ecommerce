...
<nav class="bg-brand-blue-700 text-[#f2ead9]">
    <div class="shell">
        ...
        <div class="grid grid-cols-2 grid-rows-2 gap-px overflow-hidden bg-white/10 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 [grid-auto-rows:0]">
            @foreach ($navCategories as $category)
                @php
                    $isActive = request()->routeIs('category.show') && request()->route('category')?->id === $category->id;
                @endphp
                <a
                    ...
                    @class([
                        ...
                        'bg-brand-blue-700 text-[#d8c79d] hover:bg-brand-blue-600 hover:text-white' => ! $isActive,
                        'bg-brand-blue-600 font-medium text-white' => $isActive,
                    ])
                >
                    ...
                    @elseif ($category->icon)
                        <img src="{{ \Illuminate\Support\Facades\Storage::url($category->icon) }}" alt="" class="size-5 shrink-0 object-contain brightness-0 invert" loading="lazy" />
                    @endif
